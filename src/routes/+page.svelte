<script lang="ts">
	...
	const stays = [
		{ name: 'Nightly', note: 'per site, per night' },
		{ name: 'Weekly', note: '7 consecutive nights' },
		{ name: 'Monthly', note: '30 days' },
		{ name: 'Seasonal', note: 'the full season' }
	];

	const rules = [
		{ icon: LogIn, label: 'Check-in', value: '2:00 PM' },
		{ icon: LogOut, label: 'Check-out', value: '12:00 PM' },
		{ icon: Moon, label: 'Quiet hours', value: '10 PM – 7 AM' },
		{ icon: Gauge, label: 'Speed limit', value: '5 MPH' }
	];
</script>
...
<section id="stay" class="relative isolate overflow-hidden">
	...
	<div class="mx-auto max-w-6xl px-5 pt-20 pb-28 sm:pt-28 sm:pb-36">
		...
		>
			<MapPin size={15} class="text-accent" /> Bartonville, Illinois
		</p>
		...
		<div class="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
			{#each stays as s (s.name)}
				<div class="rounded-2xl border border-line bg-canvas/60 p-6">
					...
					<p class="mt-5 text-sm font-medium text-brand">Call for rates</p>
				</div>
			{/each}
		</div>
	</div>
	...
	<div class="mx-auto max-w-6xl px-5 py-20 sm:py-24">
		...
		>
			...
			<p class="mt-8 flex items-center justify-center gap-2 text-sm text-pine-100/70">
				<Clock size={15} /> Office hours: Mon–Sat 8:00 AM – 6:00 PM
			</p>
		</div>
	</div>
</section>
